...
import io.drozda.sandbox.scenario.tradeflow.app.TradeFlowScenarioApplication;
import io.drozda.sandbox.scenario.tradeflow.app.TradeFlowScenarioCommandRequest;
import io.drozda.sandbox.scenario.tradeflow.app.TradeFlowScenarioStatus;

@Component
public class TradeFlowEnvironment implements ScenarioEnvironment {

    private static final String TOPIC = "scenario-002-trade-events";
    ...
    public synchronized ScenarioEnvironmentStatus start() {
        if (applicationContext == null) {
            String groupId = "scenario-002-trade-flow-" + UUID.randomUUID();
            applicationContext = new SpringApplicationBuilder(TradeFlowScenarioApplication.class)
                    ...
                    .run(
                            ...
                            "--spring.kafka.consumer.properties.spring.json.trusted.packages=io.drozda.sandbox.scenario.tradeflow.model",
                            "--spring.kafka.consumer.properties.spring.json.value.default.type=io.drozda.sandbox.scenario.tradeflow.model.TradeFlowEvent",
                            "--app.kafka.topics.trade-flow=" + TOPIC
                    );
