...
import io.drozda.sandbox.scenario.partitionordering.app.PartitionOrderingCommandRequest;
import io.drozda.sandbox.scenario.partitionordering.app.PartitionOrderingScenarioApplication;
...
public class PartitionOrderingEnvironment implements ScenarioEnvironment {
    private static final String TOPIC = "scenario-005-partition-ordering";
        ...
        if (applicationContext == null) {
            applicationContext = new SpringApplicationBuilder(PartitionOrderingScenarioApplication.class)
                    ...
                    .run(
                            ...
                            "--spring.kafka.consumer.group-id=scenario-005-ordering-" + UUID.randomUUID(),
                            "--spring.kafka.consumer.properties.spring.json.trusted.packages=io.drozda.sandbox.scenario.partitionordering.model",
                            "--spring.kafka.consumer.properties.spring.json.value.default.type=io.drozda.sandbox.scenario.partitionordering.model.OrderedOrderEvent",
                            "--app.kafka.topics.partition-ordering=" + TOPIC
                    );
