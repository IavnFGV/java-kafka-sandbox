...
import io.drozda.sandbox.scenario.topicpartitionoffsetbasics.app.TopicPartitionOffsetsCommandRequest;
import io.drozda.sandbox.scenario.topicpartitionoffsetbasics.app.TopicPartitionOffsetsScenarioApplication;
...
public class TopicPartitionOffsetsEnvironment implements ScenarioEnvironment {
    ...
    public synchronized ScenarioEnvironmentStatus start() {
        if (applicationContext == null) {
            ...
            String groupId = "scenario-003-partition-offsets-" + runId;
            applicationContext = new SpringApplicationBuilder(TopicPartitionOffsetsScenarioApplication.class)
                    ...
                    .run(
                            ...
                            "--spring.kafka.consumer.properties.spring.json.trusted.packages=io.drozda.sandbox.scenario.topicpartitionoffsetbasics.model",
                            "--spring.kafka.consumer.properties.spring.json.value.default.type=io.drozda.sandbox.scenario.topicpartitionoffsetbasics.model.PartitionedEvent",
                            "--app.kafka.topics.partition-offsets=scenario-003-partition-offsets-" + runId
                    );
