...
public class LatestConsumer implements ConsumerSeekAware {
    public static final String ID = "scenario-010-latest";
    private final OffsetResetTracker tracker;

    public LatestConsumer(OffsetResetTracker tracker) { this.tracker = tracker; }

    @KafkaListener(id = ID, autoStartup = "false", topics = "${app.kafka.topics.offset-reset}",
            groupId = "${app.kafka.groups.latest}",
            properties = ConsumerConfig.AUTO_OFFSET_RESET_CONFIG + ":latest")
    public void onEvent(ConsumerRecord<String, OffsetResetEvent> record) { tracker.received(record, "latest"); }

    @Override public void onPartitionsAssigned(Map<TopicPartition, Long> assignments, ConsumerSeekCallback callback) {
        tracker.assigned("latest");
    }
}
